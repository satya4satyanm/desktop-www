...
*/
function getPath(id) {
    var num = settings.shape == "regular" ? 0 : 
                settings.shape == "rounded" ? 1 : 2;
    var sData = shapeData[num];
...
function getShapes(id, sx) {
    ...
    var item = str.split(" ");
    var cmd = "";
    var sy = sx;  // Here I need to fix the width if needed
    ...
    effect.graphics.beginFill("rgba(255, 255, 255, 1)"); // 1 background white fill
    stroke.graphics.setStrokeStyle(1).beginStroke("rgba(0,0,0,0.5)");
    outline.graphics.setStrokeStyle(1).beginStroke("rgba(255,255,255,0.5)");
